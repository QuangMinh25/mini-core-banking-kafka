# Banking Core Service AI Guide

## Purpose

`banking-core-service` is one service inside the `mini-core-banking-kafka` workspace. It is a Gradle-based Spring Boot application named `banking-core-service` and is intended to host core banking backend behavior. The currently checked-in code is still bootstrap-stage: one Spring Boot application class, one context-load test, Gradle build files, and AI framework docs.

Do not invent banking rules, APIs, tables, or Kafka topics that are not present in code or approved requirements. When the repository does not prove something, write `Unknown / needs confirmation`.
...
- Module: `banking-core-service`
- Build tool: Gradle wrapper
- Language: Java 21
- Framework: Spring Boot 4.1.0
- Main package: `com.minh.bankingcore`
- Current entry point: `src/main/java/com/minh/bankingcore/BankingCoreServiceApplication.java`
- Current test: `src/test/java/com/minh/bankingcore/BankingCoreServiceApplicationTests.java`
...
- Sibling service in this workspace: `../notification-service`

## Architecture Rules

- Preserve the current module boundary. Do not move code between `banking-core-service` and `notification-service` unless the task explicitly requires a coordinated change.
- Treat this service as Spring Boot first. Follow existing package naming and keep new code inside `com.minh.bankingcore` unless the repository already establishes a broader convention.
...
- Any future balance, ledger, transfer, payment, customer, or audit logic in this module must be treated as high risk.
...
- Kafka producer or consumer behavior is high risk because message ordering, idempotency, and replay behavior can affect financial workflows.
- Logging and debugging are high risk if they could expose customer, account, or transaction data.
...
- Keep docs and framework guidance aligned with checked-in code, not with assumptions about future banking features.
...
- Treat banking and customer-related data as sensitive even if current source does not yet model it.
...
- Add logs only at meaningful boundaries and never include secrets or sensitive business payloads.
