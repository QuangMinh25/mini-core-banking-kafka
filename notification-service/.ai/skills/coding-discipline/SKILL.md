...
Use for any task that changes code, docs, framework guidance, SQL, or tests in `notification-service`.
...
- Do not invent controllers, services, repositories, entities, migrations, Kafka topics, security flows, or delivery integrations that are not checked in.
- Do not modify business source code, refactor application code, or change runtime config during framework-maintenance tasks.
- Treat recipient and message data as sensitive even when the current repo only contains scaffolding.
