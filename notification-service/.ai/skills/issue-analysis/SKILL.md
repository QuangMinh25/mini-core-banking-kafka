...
Understand the issue in `notification-service`, identify related files and probable causes, and report uncertainty clearly.

## Required Workflow

1. Read the default context files only.
2. Inspect the relevant source, config, test, build, and docs files.
3. Trace whether the issue is local to this service or involves `banking-core-service`, PostgreSQL, or Kafka.
4. Separate evidence from inference.
5. Report findings without changing files.

## Common Mistakes To Avoid

- Presenting likely notification behavior as confirmed behavior
